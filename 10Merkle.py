```python
# merkle_blockchain_short.py — corrected compact version
...
class MerkleTree:
    ...
    def proof(self, idx: int) -> List[Tuple[str, str]]:
        ...
        for lvl in self.levels[:-1]:
            ...
            p.append(
                (lvl[sib], 'right' if i % 2 == 0 else 'left')
            )
    ...
    def verify(
        ...
        for s, pos in proof:
            if pos == 'right':
                cur = sha256(
                    bytes.fromhex(cur) + bytes.fromhex(s)
                )
            else:
                cur = sha256(
                    bytes.fromhex(s) + bytes.fromhex(cur)
                )
...
class Blockchain:
    ...
    def valid(self) -> bool:
        for i in range(1, len(self.chain)):
            ...
            if not b.hash.startswith('0' * b.diff):
                return False
